...
{
  std::cout << "Validate token" << std::endl;
  natsConnection *nc = NatsClient::instance().connection();
  if (!nc) {
    std::cout << "no connection with nats" << std::endl;
    return;
  ...
  natsConnection_PublishRequestString(nc, "auth.validate_token", inbox,
                                      payload.c_str());
}
...
void AuthSession::handle_token_response(std::string payload) {
  auto self = shared_from_this();
  std::string response = payload;
  std::cout << "Before async_write" << std::endl;
asio::post(stream_.get_executor(), [this, self, response]() {
    std::cout << "Handler posted" << std::endl;
    asio::async_write(stream_, asio::buffer(response + "\n"),
        [this, self](boost::system::error_code ec, std::size_t bytes) {
            std::cout << "Write callback called" << std::endl;
            if (ec) handle_error("AuthSession Write: ", ec);
        });
});
}