...
QueryBuilder& QueryBuilder::where(const std::string& column, const std::string& op,
                                  const std::string& placeholder) {
  append_where(column + " " + op + " " + placeholder, "WHERE");
  return *this;
}

QueryBuilder& QueryBuilder::and_where(const std::string& column, const std::string& op,
                                      const std::string& placeholder) {
  append_where(column + " " + op + " " + placeholder, "AND");
  return *this;
}

QueryBuilder& QueryBuilder::or_where(const std::string& column, const std::string& op,
                                     const std::string& placeholder) {
  append_where(column + " " + op + " " + placeholder, "OR");
  return *this;
}

void QueryBuilder::append_where(const std::string& clause, const std::string& conjunction) {
...
QueryBuilder& QueryBuilder::order_by(const std::string& column, const std::string& direction) {
...
QueryBuilder& QueryBuilder::values(const std::vector<std::string>& placeholders) {
  ...
  switch (type_) {
    case QueryType::SELECT: {
      ...
      for (size_t i = 0; i < select_columns_.size(); ++i) {
        ...
        if (i < select_columns_.size() - 1) query << ", ";
      }
      query << " FROM " << table_;
      ...
      if (!order_clauses_.empty()) {
        ...
        for (size_t i = 0; i < order_clauses_.size(); ++i) {
          ...
          if (i < order_clauses_.size() - 1) query << ", ";
        }
      }

      if (limit_) query << " LIMIT " << *limit_;
      if (offset_) query << " OFFSET " << *offset_;
      break;
    }
    ...
    case QueryType::INSERT: {
      ...
      if (!insert_columns_.empty()) {
        ...
        for (size_t i = 0; i < insert_columns_.size(); ++i) {
          ...
          if (i < insert_columns_.size() - 1) query << ", ";
        }
        query << ")";
      }
      ...
      for (size_t i = 0; i < insert_values_.size(); ++i) {
        ...
        if (i < insert_values_.size() - 1) query << ", ";
      }
      query << ")";
      ...
      if (!returning_columns_.empty()) {
        ...
        for (size_t i = 0; i < returning_columns_.size(); ++i) {
          ...
          if (i < returning_columns_.size() - 1) query << ", ";
        }
      }
      break;
    }
    ...
    case QueryType::UPDATE: {
      ...
      for (size_t i = 0; i < set_clauses_.size(); ++i) {
        ...
        if (i < set_clauses_.size() - 1) query << ", ";
      }
