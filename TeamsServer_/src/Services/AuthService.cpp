...
void AuthService::handle_register(const std::string &payload,
  ...
  if (!response.has_value()) {
    ...
    result = "{"
             ...
             ","
             "\"username\":\"" +
             ServicesTools::jsonEscape(response->username) +
             "\","
             "\"email\":\"" +
             ServicesTools::jsonEscape(response->email) +
             "\","
             "\"token\":\"" +
             ServicesTools::jsonEscape(response->token) +
             "\","
             "\"status\":\"" +
             ServicesTools::jsonEscape(response->status) +
             "\""
             "}"
             "}";
  }
...
void AuthService::handle_validate_token(const std::string &payload,
  ...
  std::string result;
  if (!response.has_value()) {
    result = R"({"error":"Login failed: invalid token."})";
  } else {
    result = "{"
             "\"data\":{"
             "\"id\":" +
