...
TcpListenerAuth::TcpListenerAuth(asio::io_context& io_context, ssl::context& ssl_ctx,
...
void TcpListenerAuth::do_accept() {
  acceptor_.async_accept([this](boost::system::error_code ec, tcp::socket socket) {
    if (!ec) {
      ...
      auto session = std::make_shared<AuthSession>(std::move(socket), ssl_ctx_, authHandler_);
