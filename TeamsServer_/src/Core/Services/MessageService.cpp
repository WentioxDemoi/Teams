...
std::optional<std::string> MessageService::sendMessage(const std::string& payload) {
  ...
  // msg.id = Crypto::generate_uuid_v7(); // A ajouter plus tard avec un vrai système d'ajout local
  // côté client. (UUID tmp coté client à remplacer par UUID généré côté serveur)
  ...
  // Ici avec le return de sendMessage, on pourrait faire le "double check" de whatsapp pour savoir
  // si le message a été distribué au client
...
std::optional<std::string> MessageService::loadConversations(const std::string& userUuid) {
  std::optional<std::vector<Message>> conversations =
      messageRepo_->findConversationsByUserUuid(userUuid);

  if (!conversations.has_value()) {
    return std::nullopt;
  }
  return ResponseFormater::format_conversations_response("conversations_loaded",
                                                         conversations.value());
}
