...
std::optional<std::string> ContactService::resolveUserByUuid(const std::string& payload) {
  ...
  user.value().status =
      contactSessionRegistry_->hasContactSession(user.value().uuid) ? "Online" : "Offline";

  std::string response =
      ResponseFormater::format_user_response("resolve_user_response", user.value());
...
std::optional<std::string> ContactService::loadContacts(const std::string& payload) {
  ...
  for (User& contact : contacts) {
    contact.status =
        contactSessionRegistry_->hasContactSession(contact.uuid) ? "Online" : "Offline";
  }

  return ResponseFormater::format_user_list_response("contacts_loaded", contacts);
}

std::optional<std::string> ContactService::searchUsers(const std::string& userUuid,
...
std::optional<std::string> ContactService::lastReadAt(const std::string& userUuid,
                                                      const std::string& payload) {
...
std::optional<std::string> ContactService::updateStatus(const std::string& userUuid,
                                                        const std::string& payload) {
  const std::string status = PacketHelper::extractValue(payload, "status");
  ...
  const std::string notification =
      ResponseFormater::format_contact_status_update_response(userUuid, status);

  for (const User& recipient : recipients) {
    if (contactSessionRegistry_->hasContactSession(recipient.uuid)) {
      contactSessionRegistry_->sendMessage(recipient.uuid, notification);
    }
  }
