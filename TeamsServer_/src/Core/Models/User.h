...
struct User {
  ...
  std::string
      plain_password;  // mot de passe en clair, transitoire — JAMAIS persisté ni renvoyé au client
  std::string token;
  ...
  User(std::string uuid_, std::string firstName_, std::string lastName_, std::string email_,
...
inline User user_from_db_row(const pqxx::row& row) {
  ...
  user.token_expires_at =
      Config::instance().string_to_time_point(row["token_expires_at"].as<std::string>());
  return user;
}
