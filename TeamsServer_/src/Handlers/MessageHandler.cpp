...
void MessageHandler::handle_send_message(std::string uuid, std::string payload,
  ...
  asio::post(worker_pool_, [this, payload, respond]() {
    try {
      ...
      if (!response.has_value()) {
        response =
            R"({"type":"message_sent","error":"Send failed: invalid data or unauthorized."})";
      }
...
void MessageHandler::handle_load_conversations(std::string uuid, std::string payload,
                                               ResponseCallback respond) {
  asio::post(worker_pool_, [this, uuid, respond]() {
    try {
      ...
      if (!response.has_value()) {
        response =
            R"({"type":"conversation_response","error":"Load failed: invalid token or user."})";
      }
