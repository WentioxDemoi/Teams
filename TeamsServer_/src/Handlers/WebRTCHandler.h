...
class WebRTCHandler {
 public:
  ...
  WebRTCHandler(std::unique_ptr<IWebRTCService> webRTCService)
      : worker_pool_(Config::instance().worker_pool_size()),
        webRTCService_(std::move(webRTCService)) {}
 ...
 private:
  ...
  void handle_camera_enabled_change(const std::string& uuid, std::string payload,
                                    ResponseCallback respond);
  void handle_signaling_relay(const std::string& uuid, std::string payload,
                              ResponseCallback respond);

  bool relay_to_target(const std::string& senderUuid, const std::string& targetUuid,
                       std::string payload);
