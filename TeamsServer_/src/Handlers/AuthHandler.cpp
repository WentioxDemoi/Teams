  ...
  asio::post(worker_pool_, [this, user, respond]() {
    try {
      ...
      if (!response.has_value()) {
        ...
        result = ResponseFormater::format_user_response("login_response", *response);
      }
      ...
      std::string error = R"({"type":"login_response","error":"Login failed: server error"})";
      respond(error);
    }
  });
}

void AuthHandler::handle_register(std::string payload, ResponseCallback respond) {
  User user = user_from_json(payload);
  ...
  asio::post(worker_pool_, [this, user, respond]() {
    try {
      ...
      if (!response.has_value()) {
        ...
        result = ResponseFormater::format_user_response("register_response", *response);
      }
      ...
      std::string error = R"({"type":"register_response","error":"Register failed: server error"})";
      respond(error);
    }
  });
}
  ...
  asio::post(worker_pool_, [this, token, respond]() {
    try {
      ...
      if (!response.has_value()) {
        ...
        result = ResponseFormater::format_user_response("validate_token_response", *response);
      }
      respond(result);
