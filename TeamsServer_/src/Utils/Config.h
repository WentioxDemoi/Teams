...
class Config {
 public:
  ...
  std::chrono::system_clock::time_point token_expiry_time() const {
    return std::chrono::system_clock::now() + std::chrono::minutes(token_expiry_minutes_);
  }

  std::chrono::system_clock::time_point time() const { return std::chrono::system_clock::now(); }
