...
std::string sendAndRead(ssl::stream<tcp::socket> &stream,
...
TEST(NetworkServerIntegrationTest, AuthListenerPerformsTlsHandshakeAndDelegatesLogin) {
  ...
  auto listener = std::make_unique<TcpListenerAuth>(
      io, serverContext, tcp::endpoint(tcp::v4(), 19080), handler);
  IoGuard ioGuard(io);
...
TEST(NetworkServerIntegrationTest, ContactListenerAuthenticatesAndRegistersSession) {
  ...
  configureServerContext(serverContext);
  auto authService = std::make_shared<MockAuthService>();
  EXPECT_CALL(*authService, validateToken("token-1"))
      .WillOnce(Return(authenticatedUser()));
  ...
  auto listener = std::make_unique<TcpListenerContact>(
      io, serverContext, tcp::endpoint(tcp::v4(), 19084), contactHandler,
  ...
  EXPECT_EQ(registeredFuture.wait_for(std::chrono::seconds(2)),
...
TEST(NetworkServerIntegrationTest, MessageListenerAuthenticatesAndRegistersSession) {
  ...
  EXPECT_CALL(*registry, registerMessageSession("u1", _))
      .WillOnce([&registered](const auto &, const auto &) { registered.set_value(); });
    EXPECT_CALL(*registry, unregisterMessageSession("u1"))
      .WillOnce([&unregistered](const auto &) { unregistered.set_value(); });

  auto listener = std::make_unique<TcpListenerMessage>(
      io, serverContext, tcp::endpoint(tcp::v4(), 19082), messageHandler,
      registry, authService);
  IoGuard ioGuard(io);
  ...
  EXPECT_EQ(unregisteredFuture.wait_for(std::chrono::seconds(2)),
            std::future_status::ready);
}
...
TEST(NetworkServerIntegrationTest, WebRTCListenerAuthenticatesAndRegistersSession) {
  ...
  EXPECT_CALL(*registry, register_session("u1", _))
      .WillOnce([&registered](const auto &, const auto &) { registered.set_value(); });
    EXPECT_CALL(*registry, unregister_session("u1"))
      .WillOnce([&unregistered](const auto &) { unregistered.set_value(); });

  auto listener = std::make_unique<TcpListenerWebRTC>(
      io, serverContext, tcp::endpoint(tcp::v4(), 19083), registry,
      webRTCHandler, authService);
  IoGuard ioGuard(io);

  auto client = connectTls(io, 19083);
  asio::write(*client.stream,
              asio::buffer(std::string(R"({"token":"token-1"})") + "\n"));
