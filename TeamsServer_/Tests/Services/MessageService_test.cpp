...
std::unique_ptr<MessageService> makeService(
    MockMessageRepository *&rawMessageRepo,
...
TEST(MessageServiceTest, SendMessage_MissingContent_ReturnsNulloptAndSkipsRepo) {
  ...
  auto res = svc->sendMessage(
      R"({"senderUuid":"u1","receiverUuid":"u2"})");
...
TEST(MessageServiceTest, SendMessage_SaveFails_ReturnsNullopt) {
  ...
  auto res = svc->sendMessage(
      R"({"senderUuid":"u1","receiverUuid":"u2","content":"hello"})");
  EXPECT_FALSE(res.has_value());
}
