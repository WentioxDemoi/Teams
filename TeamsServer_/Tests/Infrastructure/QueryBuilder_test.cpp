...
TEST(QueryBuilderTest, UsesDefaultSelectColumn) {
  EXPECT_EQ(QueryBuilder().select().from("users").build(),
            "SELECT * FROM users");
}
...
TEST(QueryBuilderTest, BuildsInsertWithColumnsAndReturning) {
  ...
  const auto query = builder
      .insert_into("users", {"email", "password_hash"})
      .values({"$1", "$2"})
...
TEST(QueryBuilderTest, BuildsInsertWithoutOptionalClauses) {
  ...
  EXPECT_EQ(builder.insert_into("users").values({"$1"}).build(),
...
TEST(QueryBuilderTest, BuildsDeleteWithFiltersAndReturning) {
  ...
  EXPECT_EQ(builder.delete_from("users")
                .where("id", "=", "$1")
                .returning({"id"})
                .build(),
...
TEST(QueryBuilderTest, ResetClearsAllClauses) {
  ...
  EXPECT_EQ(builder.select().from("sessions").build(),
