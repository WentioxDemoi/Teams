...
class MockUserRepository : public IUserRepository {
 public:
  MOCK_METHOD(std::optional<User>, find_by_uuid, (const std::string& uuid), (override));

  MOCK_METHOD(std::optional<User>, find_by_email, (const std::string& email), (override));

  MOCK_METHOD(std::optional<User>, find_by_token, (const std::string& token), (override));

  MOCK_METHOD(std::vector<User>, search_by_name,
              (const std::string& callerUuid, const std::string& name), (override));
